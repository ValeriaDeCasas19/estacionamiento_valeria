        ...
        conexion.query(
          ...
          (fallo) => {
            if (fallo) {
              ...
              return res
                .status(500)
                .json({ error: "Error al registrar entrada" });
            }
            console.log(
              `Entrada registrada para ${placas} a las ${fecha_entrada}`
            );
...
exports.registrarSalida = (req, res) => {
  ...
  conexion.query(consulta, [placas], (fallo, resultados) => {
    ...
    if (tipo === "Residente") {
      pago = minutos * 1;
    } else if (tipo === "No residente" || tipo === "No residente") {
      pago = minutos * 3;
    } else if (tipo === "Vehiculo oficial") {
      pago = 0;
    }
